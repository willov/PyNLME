...
pub fn safe_inverse(matrix: &Array2<f64>) -> Result<Array2<f64>, NLMEError> {
    ...
    let inverse = na_matrix.try_inverse().ok_or(NLMEError::SingularMatrix)?;
...
pub fn safe_cholesky(matrix: &Array2<f64>) -> Result<Array2<f64>, NLMEError> {
    ...
    let chol = na_matrix.cholesky().ok_or(NLMEError::SingularMatrix)?;
...
pub fn mvn_logpdf(
    ...
    let cov_inv = cov_matrix.try_inverse().ok_or(NLMEError::SingularMatrix)?;
...
pub fn sample_mvn(
    ...
    let z: Array1<f64> = (0..k).map(|_| normal.sample(rng)).collect();
